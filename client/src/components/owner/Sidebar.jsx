...
const Sidebar = () => {
    ...
    max-w-13 md:max-w-60 w-full border-r border-borderColor text-sm'>
                    ...
                    <img
                        src={
                            image
                                ...
                                : user?.image
                                    ? `${user.image}?t=${Date.now()}` //  Add timestamp to bypass cache
            ...
            text-primary cursor-pointer' onClick={updateImage} >Save
                    <img src={assets.check_icon} width={13} alt="check icon" /></button>
            )}
            <p className='mt-2 text-base max-md:hidden'>{user?.name || "Guest"}</p>
